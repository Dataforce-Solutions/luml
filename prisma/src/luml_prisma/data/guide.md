...
This file documents the tools, output contracts, and conventions available in the `.prisma/` directory. Refer to this guide for CLI usage, expected file formats, and experiment tracking conventions.
...
- Any stale `.prisma/result.json` from the implement phase is cleared before the run starts. The run command must write a fresh `result.json` to report metrics and experiment IDs.
- If no `result.json` is written, the run node falls back to the exit code (0 = success) and parses stdout for metrics.

---

## `.prisma/result.json` Schema
...
## `.prisma/artifact.luml` — Model Artifact for Upload

When a workflow is configured with collection upload, the system uploads the file at `.prisma/artifact.luml` in the worktree root. This must be a LUML-format model (`.luml` TAR archive) created with `save_sklearn()`, `save_lightgbm()`, or equivalent.
...
2. `.prisma/artifact.luml` exists in the worktree
3. The workflow has a collection configured

---

## `.prisma/fork.json` Schema
