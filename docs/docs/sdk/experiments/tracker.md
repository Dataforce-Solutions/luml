...
```python
def get_experiment(experiment_id: str) -> ExperimentData | None
```
...
```python
def get_attachment(name: str, experiment_id: str | None = None) -> bytes
```
...
<a id="luml.experiments.tracker.ExperimentTracker.get_model_card"></a>
...
- `model_id` _str_ - Unique identifier of the model.
